...
def notify_slate(predictions: dict[str, Any], sport_key: str, when: str) -> None:
    ...
    if not games:
        send_telegram(f"📣 No {sport_key.upper()} games in the requested window ({when}).")
        return
    lines = [f"🗓 {sport_key.upper()} slate ({when})"]
    for dt, away, home in games:
        lines.append(f"- {away} @ {home} • {dt.strftime('%Y-%m-%d %H:%M UTC')}")
    send_telegram("\n".join(lines))
