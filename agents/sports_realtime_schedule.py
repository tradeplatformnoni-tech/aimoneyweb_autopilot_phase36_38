...
class RealTimeScheduleFetcher:
    ...
    def fetch_sofascore_schedule(self, sport: str, days: int = 3) -> list[dict[str, Any]]:
        ...
        for offset in range(days + 1):
            ...
            url = f"https://api.sofascore.com/api/v1/sport/{sport_id}/scheduled-events/{target_date}"
    ...
    def fetch_rapidapi_schedule(self, sport: str, days: int = 3) -> list[dict[str, Any]]:
        ...
        headers = {
            ...
            "X-RapidAPI-Host": "api-odds.p.rapidapi.com"
        }
    ...
    def normalize_game(self, event: dict[str, Any], sport: str, source: str) -> dict[str, Any] | None:
        ...
        try:
            if source == "sofascore":
                ...
                if commence_time:
                    try:
                        scheduled = datetime.fromisoformat(commence_time.replace("Z", "+00:00")).isoformat()
                    except:
                        scheduled = datetime.now(UTC).isoformat()
                else:
                    scheduled = datetime.now(UTC).isoformat()
...
def fetch_realtime_schedules(sports: list[str] = None) -> dict[str, list[dict[str, Any]]]:
    ...
    for sport in sports:
        ...
        print(f"[realtime_schedule] {sport.upper()}: Found {len(games)} games for today", flush=True)
