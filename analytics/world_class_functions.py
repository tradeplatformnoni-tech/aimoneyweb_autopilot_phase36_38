...
TEAM_CITIES: dict[str, dict[str, Any]] = {
    # NBA
    "Los Angeles Lakers": {"city": "Los Angeles", "timezone": "PST", "lat": 34.0522, "lon": -118.2437},
...
def calculate_head_to_head(
    ...
    home_wins = 0
    for game in last_10:
        if game.get("home_team") == home_team and game.get("home_score", 0) > game.get("away_score", 0):
            home_wins += 1
        elif game.get("home_team") == away_team and game.get("away_score", 0) > game.get("home_score", 0):
            home_wins += 1
...
def calculate_travel_impact(
    ...
    travel_distance = haversine_distance(
        away_city.get("lat", 0), away_city.get("lon", 0),
        home_city.get("lat", 0), home_city.get("lon", 0),
    )
