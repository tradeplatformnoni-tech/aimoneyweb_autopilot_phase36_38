...
RAPIDAPI_KEY = os.getenv("RAPIDAPI_KEY", "f89c81c096msh0e367842c4a9cedp172050jsn8f96a4f06504")  # For injury data and free soccer APIs (optional, with free fallbacks)

# Free Soccer API endpoints via RapidAPI
PINNACLE_ODDS_BASE = "https://pinnacle-odds.p.rapidapi.com"
FREE_LIVE_FOOTBALL_BASE = "https://free-api-live-football-data.p.rapidapi.com"
        ...
        def find_team_in_children(children, team_search_name):
            ...
            for item in children:
                if isinstance(item, dict):
                    ...
                    if team_search_name.lower() in team_name_espn.lower() or team_name_espn.lower() in team_search_name.lower():
...
def fetch_team_record_for_soccer(team_name: str) -> dict[str, Any] | None:
    """
    Fetch soccer team record (placeholder - returns None for now).
    Can be enhanced with API-Football or TheSportsDB in the future.
    """
    # TODO: Implement soccer record fetching from API-Football or TheSportsDB
    return None
                                ...
                                if summary and "-" in summary:
                                    ...
                                    try:
                                        ...
                                        if len(parts) == 2:
                                            ...
                                            total = wins + losses
                                            if total > 0:
                                                LOGGER.info(f"✅ Found record for {team_name}: {wins}-{losses}")
                                                return {
                                                    "wins": wins,
                                                    "losses": losses,
                                                    "win_pct": wins / total
...
def fetch_api_football_schedule(sport: str) -> list[dict[str, Any]]:
    ...
    # Note: API-Football supports multiple sports
...
def fetch_sofascore_soccer_schedule(sport: str) -> list[dict[str, Any]]:
    """
    Fetch soccer fixtures from SofaScore API (FREE - no API key needed!).
    
    Args:
        sport: Sport name (should be "soccer")
    
    Returns:
        List of game dictionaries
    """
    if not requests or sport.lower() != "soccer":
        return []
    
    try:
        from analytics.sofascore_client import SofaScoreClient
        
        # SofaScore uses "football" for soccer
        client = SofaScoreClient("football")
        events = client.fetch_events(days=0)  # Today only
        
        games = []
        for event in events:
            try:
                home_team = event.get("homeTeam", {}).get("name", "") if isinstance(event.get("homeTeam"), dict) else ""
                away_team = event.get("awayTeam", {}).get("name", "") if isinstance(event.get("awayTeam"), dict) else ""
                event_id = event.get("id", "")
                
                # Get scheduled time
                start_timestamp = event.get("startTimestamp", "")
                if start_timestamp:
                    scheduled = datetime.fromtimestamp(start_timestamp, tz=timezone.utc).isoformat()
                else:
                ...
                if home_team and away_team:
                    games.append({
                        "id": str(event_id),
                        "game_id": f"soccer_sofascore_{event_id}",
                        "home_team": home_team,
                        "away_team": away_team,
                        "scheduled": scheduled,
                        "sport": "soccer",
                ...
                LOGGER.warning(f"Error parsing SofaScore event: {e}")
                continue
        
        LOGGER.info(f"Fetched {len(games)} games from SofaScore for soccer")
        return games
        
    except ImportError:
        LOGGER.warning("SofaScore client not available")
    except Exception as e:
        LOGGER.error(f"Error fetching SofaScore schedule: {e}")
...
def fetch_thesportsdb_schedule(sport: str) -> list[dict[str, Any]]:
    ...
    # Note: TheSportsDB supports multiple sports and leagues
...
def generate_world_class_explanation(
    ...
    if home_record and away_record:
        ...
        explanations.append(f"📊 **Current Records:** {home_team} {home_wins}-{home_losses} ({home_win_pct:.1f}%) vs {away_team} {away_wins}-{away_losses} ({away_win_pct:.1f}%)")
        
        if home_win_pct > away_win_pct:
            explanations.append(f"✅ **Record Analysis:** {home_team} has a better record, supporting home advantage.")
        elif away_win_pct > home_win_pct:
            explanations.append(f"⚠️ **Record Analysis:** {away_team} has a better record, but home advantage makes this closer.")
    else:
        explanations.append(f"⚠️ **WARNING:** Real team records not available - prediction may be inaccurate!")
    
    # Rest days analysis
    if home_rest_days == 0:
        explanations.append(f"⚠️ **Rest Days:** {home_team} is on a back-to-back (0 days rest) - significant fatigue penalty applied.")
    elif home_rest_days == 1:
        explanations.append(f"⚠️ **Rest Days:** {home_team} has only 1 day rest - fatigue may be a factor.")
    elif home_rest_days >= 3:
        explanations.append(f"✅ **Rest Days:** {home_team} has {home_rest_days} days rest - well-rested.")
    
    if away_rest_days == 0:
        explanations.append(f"⚠️ **Rest Days:** {away_team} is on a back-to-back (0 days rest) - significant fatigue penalty applied.")
    elif away_rest_days == 1:
        explanations.append(f"⚠️ **Rest Days:** {away_team} has only 1 day rest - fatigue may be a factor.")
    elif away_rest_days >= 3:
        explanations.append(f"✅ **Rest Days:** {away_team} has {away_rest_days} days rest - well-rested.")
    ...
    if travel_data:
        ...
        if distance > 1500:
            explanations.append(f"✈️ **Travel Impact:** {away_team} traveling {distance:.0f} miles - significant travel fatigue.")
        if tz_change >= 3:
            explanations.append(f"🕐 **Time Zone:** {tz_change}-hour timezone change for {away_team} - additional fatigue penalty.")
    ...
    if injuries:
        ...
        if home_injuries:
            explanations.append(f"🏥 **{home_team} Injuries:** {', '.join(home_injuries[:3])}" + ("..." if len(home_injuries) > 3 else ""))
        if away_injuries:
            explanations.append(f"🏥 **{away_team} Injuries:** {', '.join(away_injuries[:3])}" + ("..." if len(away_injuries) > 3 else ""))
    
    # Home advantage
    explanations.append("🏠 **Home Advantage:** +100 Elo bonus for home team (crowd, familiar venue, no travel).")
    
    # Elo comparison
    if elo_diff > 150:
        explanations.append(f"📊 **Team Strength:** {home_team} significantly stronger ({home_elo:.0f} vs {away_elo:.0f} Elo).")
    elif elo_diff < -150:
        explanations.append(f"📊 **Team Strength:** {away_team} significantly stronger ({away_elo:.0f} vs {home_elo:.0f} Elo).")
    else:
        explanations.append(f"📊 **Team Strength:** Close matchup ({home_elo:.0f} vs {away_elo:.0f} Elo).")
    
    # H2H
    if h2h_data and h2h_data.get("h2h_adjustment", 0) != 0:
        h2h_pct = h2h_data.get("h2h_win_pct", 0.5) * 100
        explanations.append(f"📈 **Head-to-Head:** {home_team} has won {h2h_data.get('last_10_home_wins', 5)} of last 10 meetings ({h2h_pct:.1f}%).")
    
    # Momentum
    if momentum_data:
        streak = momentum_data.get("win_streak", 0)
        if streak > 0:
            explanations.append(f"📈 **Momentum:** {home_team if streak > 0 else away_team} on {abs(streak)}-game win streak - momentum boost applied.")
        elif streak < 0:
            explanations.append(f"📉 **Momentum:** {away_team if streak < 0 else home_team} on {abs(streak)}-game losing streak - momentum penalty applied.")
    
    # Final prediction
    if home_win_prob > 0.65:
        explanations.append(f"✅ **Prediction:** {home_team} strong favorite ({home_win_prob*100:.1f}% win probability).")
    elif away_win_prob > 0.65:
        explanations.append(f"✅ **Prediction:** {away_team} strong favorite ({away_win_prob*100:.1f}% win probability).")
    else:
        explanations.append(f"⚖️ **Prediction:** Close game ({home_win_prob*100:.1f}% vs {away_win_prob*100:.1f}%).")
...
def generate_explanation(
    ...
    return generate_world_class_explanation(
        ...
        home_elo, away_elo, elo_diff, injuries,
        home_record, away_record, 3, 3, {}, {}, {}
    )
    """
    ...
    if home_record and away_record:
        ...
        explanations.append(f"📊 **Current Records:** {home_team} is {home_wins}-{home_losses} ({home_win_pct:.1f}% win rate) vs {away_team} {away_wins}-{away_losses} ({away_win_pct:.1f}% win rate)")
        
        if home_win_pct > away_win_pct:
            explanations.append(f"✅ **Record Analysis:** {home_team} has a better record than {away_team}, which supports the home team advantage prediction.")
        elif away_win_pct > home_win_pct:
            explanations.append(f"⚠️ **Record Analysis:** {away_team} actually has a better record than {home_team}. However, home advantage (+100 Elo) combined with the close records makes {home_team} the slight favorite.")
        else:
            explanations.append(f"⚖️ **Record Analysis:** Both teams have similar records, making this a close matchup where home advantage could be the deciding factor.")
    elif home_record:
        home_wins = home_record.get('wins', 0)
        home_losses = home_record.get('losses', 0)
        explanations.append(f"📊 **{home_team} Record:** {home_wins}-{home_losses}")
        explanations.append(f"⚠️ **Note:** {away_team} record not available - prediction accuracy reduced.")
    elif away_record:
        away_wins = away_record.get('wins', 0)
        away_losses = away_record.get('losses', 0)
        explanations.append(f"📊 **{away_team} Record:** {away_wins}-{away_losses}")
        explanations.append(f"⚠️ **Note:** {home_team} record not available - prediction accuracy reduced.")
    else:
        explanations.append(f"⚠️ **WARNING:** Real team records not available. Predictions are based on statistical models only and may not reflect actual team performance. Check current standings before betting!")
...
def calculate_comprehensive_prediction(game_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if home_record and away_record:
        ...
        if sport == "soccer":
            LOGGER.info(f"✅ Using REAL records: {home_team} {home_wins}-{home_draws}-{home_losses}, {away_team} {away_wins}-{away_draws}-{away_losses}")
        else:
            LOGGER.info(f"✅ Using REAL records: {home_team} {home_wins}-{home_losses}, {away_team} {away_wins}-{away_losses}")
    else:
        # Fallback: Use hash-based ratings (less accurate)
        LOGGER.warning(f"⚠️ Real records not available for {home_team} vs {away_team}, using fallback ratings")
    ...
    # Schedule strength would need opponent Elo ratings - skip for now
    ...
    explanations = generate_world_class_explanation(
        home_team, away_team, home_win_prob, away_win_prob,
        home_elo_final, away_elo_final, elo_diff,
        injuries, home_record, away_record,
        home_rest_days, away_rest_days,
        momentum_data, h2h_data, travel_data
    )
    ...
    return {
        ...
        "records": {
            "home": home_record,
            "away": away_record
        } if (home_record or away_record) else {},
        "factors_used": {
...
def get_free_sports_schedule(sport: str) -> list[dict[str, Any]]:
    ...
    # Tier 2: For soccer, try SofaScore (FREE - no API key needed!)
    if sport.lower() == "soccer" and not games:
        sofascore_games = fetch_sofascore_soccer_schedule(sport)
        if sofascore_games:
            LOGGER.info(f"✅ Found {len(sofascore_games)} games from SofaScore for {sport}")
            games.extend(sofascore_games)
    
    # Tier 3: Try API-Football (free, unlimited if API key set)
    ...
    # Tier 4: Try TheSportsDB (free public API, no key needed!)
    if not games:
...
__all__ = [
    ...
    "fetch_sofascore_soccer_schedule",
