...
try:
    ...
    print(
        "[sports_analytics] ⚠️ Analytics modules may not be in render-deployment branch", flush=True
    )
...
def process_sport(sport: str) -> dict[str, Any]:
    ...
    if not games:
        if realtime_schedules and realtime_schedules.get(sport):
            print(
                f"[sports_analytics] No historical data for {sport}, but found {len(realtime_schedules[sport])} real-time games - generating fallback predictions",
                flush=True,
            )
    ...
    if len(X) < 20:
        ...
        if realtime_schedules and realtime_schedules.get(sport):
            print(
                f"[sports_analytics] Insufficient training data ({len(X)} samples), using real-time schedules for fallback predictions",
                flush=True,
            )
            from analytics.sports_data_manager import GameRecord
...
def main() -> None:
    ...
    try:
        ...
        if not HAS_SKLEARN:
            print(
                "[sports_analytics] ⚠️ scikit-learn not available - using fallback mode", flush=True
            )
        if HAS_PANDAS and HAS_NUMPY and HAS_SKLEARN:
            print("[sports_analytics] ✅ Dependencies validated", flush=True)
        else:
            print(
                "[sports_analytics] ⚠️ Some dependencies missing - continuing with degraded functionality",
                flush=True,
            )
    except Exception as e:
        print(
            f"[sports_analytics] ⚠️ Startup validation error: {e} - continuing with fallback",
            flush=True,
        )
        import traceback
        ...
        try:
            for sport in SPORTS:
                ...
                try:
                    ...
                    print(
                        f"[sports_analytics] ⚠️ Error processing {sport}: {sport_error}", flush=True
                    )
                    import traceback
                    ...
                    if consecutive_errors >= max_consecutive_errors:
                        print(
                            f"[sports_analytics] ❌ Too many consecutive errors ({consecutive_errors})",
                            flush=True,
                        )
                        print(
                            "[sports_analytics] ⏳ Entering cooldown period (60s) - will retry after",
                            flush=True,
                        )
            ...
            if consecutive_errors >= max_consecutive_errors:
                ...
                print(
                    "[sports_analytics] ⏳ Entering extended cooldown (300s) - will retry after",
                    flush=True,
                )
